-- Parallax License Server Database Schema
...
-- Seed default tiers configuration (stored in metadata)
COMMENT ON TABLE licenses IS 'Stores license information for Parallax users';
COMMENT ON TABLE activations IS 'Tracks active license activations on different machines';
COMMENT ON TABLE validation_logs IS 'Audit log of all license validation attempts';
COMMENT ON TABLE admin_users IS 'Administrative users who can manage licenses';
COMMENT ON TABLE gumroad_sales IS 'Stores raw Gumroad sale data for license generation';
